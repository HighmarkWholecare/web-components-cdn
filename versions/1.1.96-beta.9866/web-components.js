...
`;
  ...
  var FormController = class _FormController {
    ...
    edit(data) {
      ...
      if (this.accordionGroups.size > 0) {
        this.accordionGroups.forEach((accordions) => {
          accordions.forEach((accordion) => accordion.hide());
        });
      }
      queueMicrotask(() => {
    ...
    updateButtonState() {
      ...
      if (this.accordionGroups.size > 0) {
        const isOnLastStep = this.step !== void 0 && this.step >= this.steps;
        if (isOnLastStep) {
          for (let s4 = 1; s4 <= this.steps; s4++) {
            if (!this.validateStep(s4)) {
              allowSubmit = false;
          ...
          if (this.step !== void 0 && !this.validateStep(this.step)) {
            allowSubmit = false;
          }
        }
...
`;
  ...
  var Input = class _Input extends HMWCFormComponent {
     ...
     * spaces.
     *
     * Explicitly checks `minlength` because the native constraint
     * only fires for user-edited ("dirty") values, so programmatic
     * assignments would otherwise bypass the check.
     */
    checkValidity() {
      const strVal = String(this.value ?? "");
      if (this.required && !strVal.trim()) {
        return false;
      }
      if (this.minlength != null && strVal.trim().length > 0 && strVal.length < this.minlength) {
        return false;
      }
    ...
    render() {
      ...
      const classifier = e7({
        ...
        "no-spin": this.type === "number" && (!this.max || Number(this.max) > 100) || this.minlength != null && this.maxlength != null && this.minlength === this.maxlength && this.minlength > 3,
        calendar: this.open,
        valid: !!this.valid,
